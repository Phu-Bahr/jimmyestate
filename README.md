...
Jimmy Estate is a personal CMS app. My end goal was to create an application that would give the user most of the control on the content in their site like other CMS websites but without the additional costs. Currently it's geared towards Real Estate Agents but can be customized for any type of use.

The goal of this project was to learn how to implement a full application. Everything from the front end logic, the CSS displays, and responsiveness to the back end validations, data management, and http requests. It was also a great way to practice communicating with a client's needs and experiencing the almost infinite amount of edge cases possible. 
...
- Add and edit client testimonials
- Edit your Portfolio with new listings from current, sold, or rented
...
1. In the terminal, be in the location you want to clone the project. Desktop, folder, etc, type in the below followed by a name of the app you want, in this case it's *my-real-estate-app*.\
   `git clone http://github.com/Phu-Bahr/jimmyestate.git my-real-estate-app`

2. Once cloned, cd into the folder of your app.

3. Back in GitHub, create a repository, name it, choose public or private, and leave the rest of the boxes blank.

4. Back in your command line, if you type `git remote -v`, you'll see the app is still connected to the original repo.

5. Type `git remote rm origin` to disconnect it. We then want to connect it to your own repo you just created by typing this in your terminal; `git remote add origin https://github.com/USERNAME/my-real-estate-app.git`.

6. `git remove -v` again and see if it connected correctly to your repo. If it is, then `git push origin master`.
  
### **Setting up Dev Environment**

1. In the terminal (make sure you're in the my-real-estate-app folder) type `bundle install` to install all the gems in Gemlock file.
...
4. Open the app in your text Editor. I'm using VSCode. Go to `config/environments/development.rb` file and comment out lines 43 to 50 for now. We're going to comment this back in later.
...
8. Now that we have a database and tables, lets fill it with some sample data. Type in `rails db:seed`. This is going to take our seed file with sample data and add it to our tables.

9. The last step is to set up a User with admin rights. In your command line again, type `rails c`. Info to rails console here [Link](https://guides.rubyonrails.org/command_line.html "Rails Intro"). Basically, in rails c you can test out rails app without being in the site. Think of it as entering CURL commands but for rails. We're in here to create a user and add it to the database.
...
``` 
Admin: true is the main part we need. Type `exit` to exit out of the rails c session.
...
2. Once you have your key, you need to make sure to restrict it's access so others won't use it maliciously. It won't be hidden in your code, so restricting it online is best practice. Read more best practices [here](https://developers.google.com/maps/api-key-best-practices#restrict_apikey "API best practice").
...
5. Under Website restrictions, add `localhost:3000/*` and a heroku name while we're still here. e.g. `my-real-estate-app.heroku.com/*`. When your site is live take off 3000 or change your 3000 path to something else so others can't use your api in their dev environment.
...
5. By doing this, you've created a new `master.key` file, and a new `credentials.yml` file that should be open in a new tab for you. the --wait command will still be running in the terminal until you save and close the credentials file. Enter the below code, save and close tab.
...
2. First create a new Google Account or user current account and reduce its security measures for your app to work.
   ...
   - Click on `Generate` and your new password will appear. Copy this. We need to put it in our `credentials.yml file.
...
4. Now we can undo the commenting we did earlier in `config/environments/development.rb` from line 43 to 50, as this block now has information it needs to work.
...
5. Restart Rails s and webpacker to test your email out in one of the contact forms.
...
2. Once we have an API key, we need to restrict it per domain. Go to [admin link](https://www.google.com/recaptcha/admin "admin recaptcha"), click on the `settings cog` and add your domains. Mainly localhost:3000 and your heroku one, i.e. my-real-estate-app.herokuapp.com.
...
4. Now that we have an API key, go to `app/javascript/Constants/Constants.jsx` and change the variable on line 394 with your recaptcha api key. This won't be hidden, however that's why you restricted it per domain.
...
1. If you want to use this, it'll track all pages users visit as well as any clicks on your site, desktop or mobile. 
...
3. Once you have your UA-#########-# ID, go to `app/javascript/packs/index.jsx` file. This is the topmost file when rendering your React App. Plug in your tracking ID.
...
4. With all the google tools added, lets commit the changes to GitHub:
```bash
git add .
...
2. Create an Heroku app in Heroku, choose an app name (we probably chosen it by now from the set up before i.e. ***`my-real-estate-app`**.herokuapp.com*)
